...
import random
# custom functions
from scrap_functions import click_element_by_xpath, send_key_by_name,  click_element_by_id, get_element_by_id, get_element_by_xpath, send_key_by_id
...
def run_webscraping(url=None, logging_check=False):
    ...
    username = "nepalsingh@yahoo.com"
    password = "N3wP@5s123!"
    # username = "XY40"
    # password = "123123"

    search_element = '//*[@id="home-search-box"]'
    search_term = "Doheny SB"

    searchbos = get_element_by_xpath(
        driver=driver, search_key=search_element, sleep_time=5, debug=True)

    print('searchbos:', searchbos)
    # searchbos.send_keys(search_term)
    # searchbos.send_keys(Keys.RETURN)

    time.sleep(2)

    search_element = '//*[@id="login-btn-mobile"]'
    btn_xpath = get_element_by_xpath(
        driver=driver, search_key=search_element, sleep_time=5, debug=True)

    print('Btn_xpath:', btn_xpath, ' text : ', btn_xpath.text, ' tag_name : ',
          btn_xpath.tag_name, type(btn_xpath))

    btn_xpath.click()

    time.sleep(5)

    print('Btn_xpath:', btn_xpath, btn_xpath.text,
          btn_xpath.tag_name, type(btn_xpath))
    btn_xpath.click()
...
url = "https://www.reservecalifornia.com/Web/Default.aspx"

run_webscraping(url=url, logging_check=False)
